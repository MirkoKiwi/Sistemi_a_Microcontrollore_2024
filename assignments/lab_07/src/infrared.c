...
void CaptureIRSignal() {
    ...
    if (current_state != previous_state) {
        ...
        if (!capturing && signal_duration > US_TO_TICKS(8800) && signal_duration < US_TO_TICKS(9200)) {
            // Impulso di start rilevato (9 ms)
            start_sequence++;
        }
        else if (start_sequence == 1 && signal_duration > US_TO_TICKS(4400) && signal_duration < US_TO_TICKS(4600)) {
...
void DecodeNECProtocol(int signal_duration) {
    if (bit_count < 32) {
        if (signal_duration > US_TO_TICKS(1600)) {
            // "1" logico
            data = (data << 1) | 1;
        } else if (signal_duration > US_TO_TICKS(400) && signal_duration <= US_TO_TICKS(1600)) {
            // "0" logico
            data = (data << 1);
        } else {
            // Durata non valida, interrompe la cattura
            xil_printf("Errore: Segnale fuori range\n");
            capturing = 0;
            return;
        }
